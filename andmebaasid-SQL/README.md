...
Selles repositooriumis on kogumik õpingute jooksul valminud näidisandmebaase (MS Access `.accdb`) ja SQL-skripte, mis käsitlevad relatsiooniliste andmebaaside kavandamist ja normaliseerimist ning SQL-päringute koostamist. Näited varieeruvad alates andmebaasi ja tabelite loomisest kuni keerukamate päringutüüpideni nagu `JOIN`, `UNION`, `INTERSECT`, `NOT EXISTS` ja rekursiivsed (iseendaga seotud) tabelid.
